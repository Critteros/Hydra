export enum PrismaErrorCode {
  ...
  /**
   * The record searched for in the where condition (`{model_name}.{argument_name} =
   * {argument_value}`) does not exist
   */
  ...
  /**
   * The value `{field_value}` stored in the database for the field `{field_name}` is invalid for
   * the field's type
   */
  ...
  /**
   * The change you are trying to make would violate the required relation '{relation_name}' between
   * the `{model_a_name}` and `{model_b_name}` models.
   */
  ...
  /**
   * The records for relation `{relation_name}` between the `{parent_name}` and `{child_name}`
   ...
   * {@link http://pris.ly/d/connection-pool} (Current connection pool timeout: {timeout}, connection
  ...
  /**
   * Cannot find a fulltext index to use for the search, try adding a @@fulltext([Fields...]) to
   * your schema
   */
  NoFulltextIndex = 'P2030',

  /**
   * Prisma needs to perform transactions, which requires your MongoDB server to be run as a replica
   * set. See details: {@link https://pris.ly/d/mongodb-replica-set}
   */
